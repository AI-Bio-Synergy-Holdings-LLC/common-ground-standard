# Common Ground Standard

Public review drafts for responsible AI infrastructure, campus AI governance, AI benefit clauses, evidence methods, and anti-capture safeguards.

## Purpose

The Common Ground Standard is a public-interest standards effort for making AI deployment and AI-enabling infrastructure prove measurable human, community, worker, student, and ecological benefit.

This repository is a public review surface. It is meant for critique before broader outreach to foundations, agencies, AI companies, infrastructure operators, or data-center actors.

## Public Doctrine

- No AI deployment without human agency.
- No AI infrastructure without community benefit.
- No AI productivity gain without a dividend path.
- No data extraction without data-trust governance.
- No ecological burden without transparency, restoration, and enforceable guardrails.
- No AI education system without student authorship and learning integrity.

## Review Drafts

Start here:

1. [Review Brief](REVIEW_BRIEF.md)
2. [Common Ground Standard for Responsible AI Infrastructure](standards/common-ground-standard.md)
3. [Campus Common Ground Compact](standards/campus-common-ground-compact.md)
4. [AI Dividend / Benefit Clause](standards/ai-dividend-benefit-clause.md)
5. [Reviewer Questions](review/reviewer-questions.md)
6. [Anti-Capture Review Checklist](review/anti-capture-checklist.md)
7. [Public Roadmap Summary](ROADMAP.md)
8. [Public Review Notice](NOTICE.md)

## Current Status

This project is:

- public-review ready;
- standards-draft ready;
- seeking critique from legitimacy actors before industry outreach.

This project is not yet:

- certified;
- operationally proven at scale;
- endorsed by reviewers;
- claiming active pilots unless separately documented;
- claiming agency, foundation, university, tribal, community, or company partnership.

## Review Ask

Please review the drafts and identify:

- where the standards are too vague to prevent capture;
- where student, worker, community, data, or ecological safeguards need more force;
- what minimum evidence should be required before any actor can claim alignment;
- what should trigger no-go conditions;
- what would make this credible before industry or infrastructure actors are invited in.

## How To Review

- Read [Review Process](REVIEW_PROCESS.md)
- Read [Contributing](CONTRIBUTING.md)
- Open a structured issue using one of the templates:
  - Standard feedback
  - Anti-capture risk
  - Campus Compact feedback
  - AI Benefit Clause feedback
  - Evidence requirement

## Notice

Reviewing this repository does not create endorsement, partnership, certification, ownership transfer, data access, model-training rights, or permission to use marks.

See:

- [Public Review Notice](NOTICE.md)
- [License and Trademark Notice](LICENSE-NOTICE.md)
